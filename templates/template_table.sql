...
create table change_me (
 ...
 ,is_active      number(1,0)    default 1 not null
  -- Colunas de auditoria padrao (Guideline G-2340)
...
-- Comentarios (Guideline G-2150: documentar todos os objetos)
comment on table change_me is 'TODO: descricao da tabela';
comment on column change_me.change_me_id is 'Identificador unico (PK identity)';
comment on column change_me.change_me_code is 'Codigo unico de negocio (UK)';
comment on column change_me.change_me_name is 'Nome descritivo';
comment on column change_me.change_me_desc is 'Descricao detalhada';
comment on column change_me.change_me_seq is 'Sequencia de ordenacao';
comment on column change_me.is_active is 'Flag de ativo: 1=Sim, 0=Nao';
comment on column change_me.created_on is 'Data/hora de criacao do registro';
comment on column change_me.created_by is 'Usuario que criou o registro';
comment on column change_me.updated_on is 'Data/hora da ultima atualizacao';
comment on column change_me.updated_by is 'Usuario da ultima atualizacao';
...
-- Guideline G-2120: usar check constraints para validacao de dominio
...
-- Foreign keys (descomentar e ajustar conforme necessario)
-- alter table change_me add constraint change_me_fk1
--   foreign key (parent_id) references parent_table(parent_id);

-- Indices para foreign keys (Guideline G-2130: indexar colunas de FK)
